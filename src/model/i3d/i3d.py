...
class Unit3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if isinstance(self.padding, tuple):
            # Tuple padding: (pad_t, pad_h, pad_w) -> (pad_w_f, pad_w_b, pad_h_f, pad_h_b, pad_t_f, pad_t_b)
...
class InceptionModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, name):
        ...
        self.b1b = Unit3D(in_channels=out_channels[1], output_channels=out_channels[2], kernel_shape=[3, 3, 3], padding=1, name=name+'/Branch_1/Conv3d_0b_3x3')
        ...
        self.b2b = Unit3D(in_channels=out_channels[3], output_channels=out_channels[4], kernel_shape=[3, 3, 3], padding=1, name=name+'/Branch_2/Conv3d_0b_3x3')
...
class I3D(nn.Module):
    ...
    def __init__(
        ...
        self.conv3d_2b_1x1 = Unit3D(in_channels=64, output_channels=64, kernel_shape=[1, 1, 1], padding=0, name=name+'/Conv3d_2b_1x1')
        self.conv3d_2c_3x3 = Unit3D(in_channels=64, output_channels=192, kernel_shape=[3, 3, 3], padding=1, name=name+'/Conv3d_2c_3x3')
        ...
        self.dropout = nn.Dropout(1.0 - dropout_keep_prob)  # Convert keep_prob to drop_prob
        self.logits = Unit3D(
            in_channels=1024,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.spatial_squeeze:
            x = x.squeeze(3).squeeze(3).squeeze(2)  # (batch, num_classes)
